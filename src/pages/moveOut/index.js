...
import { assignLocalDataAction, socketAssociateBulkData, socketDeviceStatus } from 'redux/actions/socketAction';
...
const MoveOut = (props) => {
    ...
    const handleCompleteMoveOut = () => {
        ...
        // moveInData.push (
        //    {
        //        unitId: assignable?.map((i) => i?.unitId).join(', '),
        //        productCode: assignable?.map((i) => i?.productCode).join(', '),
        //        description: assignable?.map((i) => i?.description).join(', '),
        //        expiryDate: expiryDate,
        //        tagId: assignable?.map((i) => i?.rfidNumber).join(', '),
        //        //recipientId: location.pathname.split('/').pop(),
        //        dereservationDate: tableDRdate,
        //        bloodGroup: assignable?.map((i) => i?.bloodGroup).join(', '),
        //    }
        //)

        if (value.deviceTypeId?.[0]?.behaviourProperties === 'isAssociated' || value.deviceTypeId?.[0]?.behaviourProperties === 'isEmergency') {
            ...
            //{
            //    event_id: '',
            //    token: value && value?.[0]?.token,
            //    timeStamp: '',
            //    user: apiResponse.responseData.data?.[0]?.badgeNo,
            //    reader: {
            //        serial: value && value?.[0]?.serialNumber,
            //        drawer: "1"
            //    },
            //    events: [
            //        {
            ...
            <Grid container item className={classes.tableGrid}>
                <Grid container item xs={12} lg={12} md={12} className={classes.minHeight}>
                    <Grid container item>
                        ...
                        <Grid item xs={4} className={classes.addUnitsButton}>
                            <Grid item lg={3} xs={4}>
                                                    ...
                                                    backgroundColor: successTags?.includes(currentUnit?.rfidNumber)
                                                        ...
                                                        : failedTags?.includes(currentUnit?.rfidNumber)
                                                            ...
                                                            currentUnit?.assigned //#8091fa
                                                                ? currentUnit?.recipientId === recipientId
                                                                    ? '#cafccc'
                                                                    : '#8091fa'
                                                                : !currentUnit?.associated
                                                                    ? '#ff4d4d'
                                                                    : null
                                                }}
                                                key={indexm}
                                                    ...
                                                    >
                                                        {item === 'tableDRdate' ? (
                                                            <DateTimePicker
                                                                ...
                                                                // color={indexm === 0 ? CONSTANTS.COLOR_PRIMAY_MAIN : ''}
                                                                disabled={
                                                                    currentUnit?.assigned
                                                                        ? true
                                                                        : successTags?.includes(currentUnit?.rfidNumber)
                ...
                <Grid container item xs={12} lg={12} md={12}>
                    <Grid item container className={classes.assignAndBackBtns}>
                        <Grid item container xs={maxWidth600 ? 12 : 8} lg={4} md={maxWidth1040 ? 6 : 5} spacing={3}>
                            <Grid item xs={5}>
                                <CustomButton
                                    variant="contained"
