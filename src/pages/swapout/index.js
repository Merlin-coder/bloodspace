...
const SwapOut = (props) => {
    ...
    let { swapoutfiltersData } = useSelector((state) => state.getSwapoutFilter);
    console.log('swapoutfiltersData', swapoutfiltersData);
    ...
    const { wastedBatchResponse, wastedBatchLoading, wastedBatchError } = useSelector((state) => state.postWastedBatch);
    console.log('wastebatchhh', wastedBatchResponse);
    ...
    useEffect(() => {
        dispatch(getSwapoutFilter(remoteInfo?.emergencyLogin === 'TRUE' ? '6320991b7d29280d14a57145' : undefined));
    }, [options8]);
    ...
    useEffect(() => {
        ...
        socket?.on('refresh', (data) => {
            if (window.location.pathname === '/dashboard/swapout' && urlEndPoint === 'swapouts') {
                dispatch(getData('swapouts', pageSize * 3, 1));
            }
        });
    }, [socket]);
    ...
    useEffect(() => {
        let tempAccessCodes = [];
        if (userInfo?.data?.user?.['useraccessrole-code'] === 'BS-UAR-1009') {
            tempAccessCodes = [
                'BS-ACO-1030',
                'BS-ACO-1031',
                'BS-ACO-1032',
    ...
    useEffect(() => {
        ...
        //dispatch(getSwapoutFilter(urlEndPoint));
    ...
    useEffect(() => {
        if (swapoutfiltersData?.status && param.state) {
            setDashboardFilter();
    ...
    //        dispatch(clearData());
    //        dispatch(getData(urlEndPoint, 100000, 1, searchKey.trim(), showFilters));
    //    } else if (searchKey === '') {
    ...
    const setDashboardFilter = () => {
        swapoutfiltersData?.data[0]?.['trackId-name'].map((val) => {
    ...
    const handleVoucher = (row, displayConfig, name) => {
        ...
        if (props.location.pathname === '/dashboard/swapout') {
            history.push({
                pathname: `/dashboard/v/swapouts/name/${row?.deviceId[0]?.name}/null/swapouts/${row?._id}`,
                state: { currentRowName: rowName, 'previousId&name': [urlEndPoint, row?._id], voucherConfig: displayConfig }
            });
        }
        else {
            history.push({
                pathname: `/dashboard/v/${url}/${row?.name?.toLowerCase()}/${childDataTable ? childDataTable : null
                    }/${urlEndPoint}/${row._id}`,
                state: { currentRowName: rowName, 'previousId&name': [urlEndPoint, row._id], voucherConfig: displayConfig }
            });
        }
    ...
    const handleBackClick = () => {
        if (history.location.pathname === '/dashboard/request-batch') {
            history.goBack();
        } else {
    ...
    const handleUnitSearch = (e) => {
        ...
        if (e.length > 8 && e !== ' ') {
            ...
            if (sortValue?.key) {
                dispatch(getData(urlEndPoint, pageSize * 3, 0, e, undefined, sortValue));
            } else {
                dispatch(getData(urlEndPoint, pageSize * 3, 0, e, undefined));
            }
            //dispatch(getData(urlEndPoint, 30, 1, JSON.stringify(e)));
        }
        else if (e === '') {
    ...
    const dropDownData = urlEndPoint === 'swapouts' ? 'productcode' : null;
    useEffect(() => {
        urlEndPoint === 'swapouts' && setSecondDropdown('locations');
        urlEndPoint === 'swapouts' && setThirdDropdown('devices');
        urlEndPoint === 'swapouts' && setFourthDropdown('recipient');
        urlEndPoint === 'swapouts' && setFifthDropdown('track');
    }, [urlEndPoint]);
    ...
    useEffect(() => {
        ...
        if (wastedBatchResponse && wastedBatchResponse.status === true) {
            setComments('');
            setSelectedRecords([]);
            setSelectedAllRecordsFlag(false);
            setAnchorEl(null);
            ...
            showSnackbar(true, wastedBatchResponse.message, 'success');
        } else if (wastedBatchError) {
            showSnackbar(true, wastedBatchError.errorMessage, 'error');
        }
        ...
        return () => {
            ...
            setTimeout(() => {
                dispatch(clearRequestbatchActions());
            }, 4000);
        };
    }, [
        deleteResponse,
        deleteError,
        requestbatchheaderActions,
        requestbatchheaderActionsError,
        headerLed,
        headerLedError,
        wastedBatchResponse,
        wastedBatchError
        ...
        return input?.fieldtypeId[0]?.code === 'SC-FT-001' ? (
            ...
            <Grid item xs={12} md={6} lg={6} className={classes.inputField} key={input.name}>
                ...
                <SelectOption
                    ...
                    options={
                            urlEndPoint === 'swapouts' && input.name === 'locationId'
                            ? options2?.data
                                : urlEndPoint === 'swapouts' && input.name === 'deviceId'
                                ? options3?.data
                                    : urlEndPoint === 'swapouts' && input.name === 'recipientId'
                                    ? options4?.data
                                        : urlEndPoint === 'swapouts' && input.name === 'trackId'
                                        ? options5?.data
                                        : options?.data
                    }
                    onChange={handleEditChange}
    ...
    const updateAction = (value) => {
        ...
        if (value !== 'BS-TR-5110' && value !== 'BS-TR-5119') {
            ...
            const wastedBatch = selectedRecords.map((ele) => ele);
            console.log('wasttttt', wastedBatch);
            const ref = {
                assignbatcheId: wastedBatch,
                comments: comments,
                userId: remoteInfo?.emergencyLogin === 'TRUE' ? userId : userRoleData?.data?.[0]?._id,
                deviceId: selectedRowObj.deviceId?.[0]?._id,
                locationId: selectedRowObj.locationId?.[0]?._id,
            ...
            dispatch(postWastedBatch(ref));
            window.location.reload(true);
        } else if (value == 'BS-TR-5119') {
            const transfusedBatch = selectedRecords.map((ele) => ele);
            console.log('transfusedBatch', transfusedBatch);
            const ref = {
                assignbatcheId: transfusedBatch,
                comments: comments,
                userId: remoteInfo?.emergencyLogin === 'TRUE' ? userId : userRoleData?.data?.[0]?._id,
                deviceId: selectedRowObj.deviceId?.[0]?._id,
                locationId: selectedRowObj.locationId?.[0]?._id,
            ...
            dispatch(postTransfusedBatch(ref));
            window.location.reload(true);
        }
    };
                    ...
                    <Paper elevation={0} className={classes.paper}>
                        <Grid container justify="space-between" spacing={2}>
                            <Grid item xs={1} className={classes.backButton}>
                                ...
                                <Grid item xs={4} style={{ marginRight: 670 }}> 
                    ...
                    {accessToTable ? (
                        <>
                            <Paper elevation={0} className={classes.paper}>
                                <Grid container className={classes.countContainer}>
                                    ...
                                    <Grid xs={2} style={{ display: 'flex', flexDirection: 'row-reverse' }}>
                                        ...
                                        {/*        <>*/}
                                        {/*            <FlareIcon color={isSelected ? 'primary' : ''} />*/}
                                        {/*            {headerLedLoading && (*/}
                                        ...
                                        <Menu
                                            ...
                                            onClose={
                                                requestbatchActionsLoading || wastedBatchLoading ? null : handleClose
                                            }
                                        ...
                                        >
                                            {!selectAllRecords && (
                                                <MenuItem
                                                    ...
                                                    disabled={requestbatchActionsLoading || remoteInfo?.emergencyLogin === 'TRUE'}
                                                >
                                                    Move to Assigned
                                            ...
                                            >
                                                Mark as Fated
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5101' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            <MenuItem
                                                ...
                                                disabled={wastedBatchLoading}
                                            >
                                                Mark as Wasted
                                                {wastedBatchLoading && selectedStatus === 'BS-TR-5110' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            <MenuItem
                                            ...
                                            >
                                                Mark as Dereserved
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5102' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            <MenuItem
                                            ...
                                            >
                                                Mark as Reserved
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5111' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            ...
                                            >
                                                Mark as Unassigned
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5112' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            <MenuItem
                                            ...
                                            >
                                                Mark as Emergency
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5113' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            ...
                                            >
                                                Mark as Quarantined
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5115' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            ...
                                            >
                                                Mark as Reactivated
                                                {requestbatchActionsLoading && selectedStatus === 'BS-TR-5118' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                            <MenuItem
                                            ...
                                            >
                                                Mark as Transfused
                                                {wastedBatchLoading && selectedStatus === 'BS-TR-5119' && (
                                                    <CircularProgress size="20px" style={{ marginLeft: '10px' }} />
                                                )}
                                            </MenuItem>
                                        </Menu>
                                    ...
                                    <Grid item xs={12}>
                                        {loading ? (
                                            ...
                                            <CustomTable
                                                ...
                                                isAction={requestbatchActionsLoading || wastedBatchLoading}
                            ...
                            <CustomDialog
                                ...
                                tabIndex={1}
                                loading={requestbatchActionsLoading || wastedBatchLoading}
                            >
                                {addCommentsForm}
                            </CustomDialog>
                            <UnitEditDialog
