...
import RuleType from 'pages/manage/movement/RuleType';
import AssignBatch from 'pages/assign-batch/assign-page/assign-page.container';

const AssignPageComponent = (props) => {
    ...
    useEffect(() => {
        dispatch(assignLocalDataAction(Array.from(new Set([]))));
    ...
    useEffect(() => {
        ...
        if (userInfo?.data?.user?.[userRoleCode] === 'BS-UAR-1002') {
            tempAccessCodes = ['BS-ACO-1012', 'BS-ACO-1013', 'BS-ACO-1014'];
        } else {
    ...
    const handleSearch = (searchKey) => {
        ...
        if (searchKey?.length > 2) {
            dispatch(clearData());
            if (accessableCodes?.includes('BS-ACO-1011') && accessableCodes?.includes('BS-ACO-1010')) {
                let DOB;
                ...
                if (typeof searchDate?.getMonth === 'function' && searchKey && invalidDate !== 'Invalid Date') {
                    ...
                    dispatch(getData('recipient', 0, 1, searchKey.trim(), DOB));
                }
            } else if (accessableCodes?.includes('BS-ACO-1011')) {
                let DOB;
                let invalidDate = String(searchDate);
                if (typeof searchDate?.getMonth === 'function' && searchKey && invalidDate !== 'Invalid Date') {
                    DOB = [
            ...
            } else if (accessableCodes?.includes('BS-ACO-1010')) {
                dispatch(
                    getData('recipient', 0, 1, undefined, undefined, undefined, undefined, undefined, searchKey.trim())
                );
            } else if (accessableCodes?.includes('BS-ACO-1009')) {
                dispatch(
                    getData(
                        'recipient',
                        0,
                ...
                dispatch(getData('recipient', 0, 1,undefined,undefined,undefined,undefined, searchKey.trim()));
            }
        } else if (searchKey === '') {
            dispatch(clearData());
        }
    ...
    const handleSearchDate = (e) => {
        ...
        if (typeof e?.getMonth === 'function' && searchKey && invalidDate !== 'Invalid Date') {
            ...
            accessableCodes?.includes('BS-ACO-1012') && dispatch(getData('recipient', 0, 1, searchKey, DOB));
...
export default AssignPageComponent;
