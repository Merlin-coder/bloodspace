...
import UnassignUnitComponent from '../unassign/unassignunit.component';
...
function RecepientPage() {
        ...
        data?.length > 0
            ? data.map((ele) => {
                ...
                return {
                    ...
                    tableDRdate: ele?.dereservationDate,
    ...
    console.log('expiryda', setExpiryDate);
    ...
    console.log('sequence', setSequenceNo);
    ...
    const handleSocketAssign = () => {
        ...
        console.log("unitsTable" + unitsTable.length);
        unitsTable.map((i) => {
    ...
    const feilds = ['name', 'dob', 'gender', 'bloodgroupId', 'assignedCountUnit', 'usedCount', 'mrnNumber'];
    const feildNames = {
        name: 'Name',
                ...
                setUnitsTable([
                    {
                        ...
                        tableDRdate: tempUnitId?.dereservationDate,
    ...
    const handleAlert = () => {
        ...
        if (assignable?.length === unitsTable?.length) {
            history.push('/dashboard/unassign-unit');
        }
    };
    ...
    const onChangeAutoComplete = (e, value) => {
        ...
        if (!tempIsbtCOdes.includes(value?.isbtcode)) {
            ...
            setTableDRdate(tempUnitId?.dereservationDate);
            setSequenceNo(tempUnitId?.sequenceNo ? tempUnitId?.sequenceNo : '-');
        } else {
        ...
        if (device !== '' && !assignLoading) {
            ...
            let tempUnits = assignData.map((i) => {
                return {
                    ...
                    tableDRdate: i?.dereservationDate,
    ...
    useEffect(() => {
        if (checkedRows.includes('All')) {
            let tempCheckedRows = unitsTable.map((i) => i?.donationCode);
