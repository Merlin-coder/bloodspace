...
export default function EuoDashboardBatchTable({ columns, response, groupBy, total, hospitalFilters }) {
    ...
    const route = (value, name, group) => {
        ...
        let filtersData;
        if (group?.product?.value === 'Total' && name === 'Total') {
        ...
        if (groupBy === 'Product') {
            filtersData = [
                { key: 'batchProductId._id', value: [group?.product?._id] },
                { key: 'bloodgroupId._id', value: [value?._id && value?._id] },
                hospitalFilters
            ];
            chipData = [name, group?.product?.value];
            if (name === 'Total') {
                filtersData = [{ key: 'batchProductId._id', value: [group?.product?._id] }];
                chipData = [group?.product?.value];
            } else if (group?.product?.value === 'Total') {
                filtersData = [{ key: 'bloodgroupId._id', value: [value?._id] }];
                chipData = [name];
            }
            filtersData = filtersData.filter((val) => val);
        } else if (groupBy === 'Location') {
            const arrayOfLocation = [];
            response?.filter((ele) => ele?.location?._id)?.map((ele) => arrayOfLocation.push(ele?.location?._id));
            filtersData = [
                { key: 'locationId._id', value: [group?.location?._id] },
                { key: 'bloodgroupId._id', value: [value?._id && value?._id] },
                hospitalFilters
            ];
            chipData = [name, group.location.value];
            if (group?.location?.value === 'Total' && name === 'Total') {
                filtersData = [{ key: 'locationId._id', value: arrayOfLocation }]; //"622b0878b09c7622f2c4ae12"
                chipData = [name];
            } else if (name === 'Total') {
                filtersData = [{ key: 'locationId._id', value: [group?.location?._id] }];
                chipData = [group?.location?.value];
            ...
            const arrayOfLocation = [];
            response?.filter((ele) => ele?.device?._id)?.map((ele) => arrayOfLocation.push(ele?.device?._id));
            filtersData = [
                { key: 'bloodgroupId._id', value: [value?._id && value?._id] },
                { key: 'deviceId._id', value: [group?.device?._id] },
                hospitalFilters
            ];
            chipData = [name, group.device.value];
            if (group?.device?.value === 'Total' && name === 'Total') {
                filtersData = [{ key: 'deviceId._id', value: arrayOfLocation }]; //"622b0878b09c7622f2c4ae12"
                chipData = [name];
            } else if (name === 'Total') {
                filtersData = [{ key: 'deviceId._id', value: [group?.device?._id] }];
                chipData = [group?.device?.value];
                            ...
                            return (
                                <TableRow hover role="checkbox" tabIndex={-1} key={row.code + '' + rindex}>
                                    {columns.map((column, cindex) => {
                                        const value = row[column.id]?.value || undefined;
