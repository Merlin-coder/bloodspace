import RequestBatchFilterComponent from './requestBatchFilterComponent';
...
const RequestBatchFilter = (props) => {
    ...
    let { requestBatchfiltersLoading, requestBatchfiltersData } = useSelector((state) => state.getRequestBatchFilter);
    ...
    let filtersNewDataKeys = requestBatchfiltersData && requestBatchfiltersData.data?.length > 0 && Object.keys(requestBatchfiltersData?.data[0]);
    ...
    // console.log("hellooowwwww we are in filter / request unit filter");
    useEffect(() => {
        if (
            requestBatchfiltersData &&
            requestBatchfiltersData.data &&
            requestBatchfiltersData.data?.length > 0 &&
            responseData &&
            responseData.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(requestBatchfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            tempFilterKeys.forEach((item) => {
                if (requestBatchfiltersData &&
                    requestBatchfiltersData.data &&
                    requestBatchfiltersData.data[0] &&
                    requestBatchfiltersData.data[0][item] &&
                    requestBatchfiltersData.data[0][item][0] &&
                    requestBatchfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(requestBatchfiltersData.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(requestBatchfiltersData.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(requestBatchfiltersData.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(requestBatchfiltersData.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    tempDatesObject[item] = dateItem;
                }
                if (requestBatchfiltersData.length > 0 && requestBatchfiltersData?.data[0][item][0]?.type === 'switch') {
            ...
            //for request unit
            if (isRequestBatch) {
                let collectionDateTimeObject = filtersNewDataKeys?.filter((i) => i.name === 'collectionDateAndTime')[0];
                let expiryDateTimeObject = filtersNewDataKeys?.filter((i) => i.name === 'expiryDate')[0];
                let dereservationDateObject = filtersNewDataKeys?.filter((i) => i.name === 'dereservationDate')[0];
                let tempStaticGroups = [
                    {
                        name: 'Batch Product',
                        data: [
                            'batchProductId-name',
                            ...
                            collectionDateTimeObject,
                            expiryDateTimeObject,
                            dereservationDateObject,
                        ]
                    },
                    {
                        name: 'Date',
                    ...
                    // { name: 'Recipient', data: ['recipientId-name', 'recipientId-mrnNumber'] },
                    { name: 'Status', data: ['trackId-name', 'createdBy-firstName'] },
                    {
                        name: 'Location',
                        data: ['locationId-name', 'authorityId-name', 'clientId-name', 'deviceId-name']
                    }
                ];
                setStaticGroups([...tempStaticGroups]);
            }
            console.log("filtersNewDataKeys", filtersNewDataKeys)
        }
        ...
        console.log(requestBatchfiltersData, 'requestBatchfiltersData');
    }, [requestBatchfiltersData, responseData]);
    useEffect(() => {
    ...
    const handleChangeChipData = () => {
        ...
        if (chipToDelete in chipNameAndId) {
            ...
            let filterKeysObjects = filtersNewDataKeys
    ...
    useEffect(() => {
        ...
        if (displayConfData) {
            displayConfData.forEach((item) => {
                ...
                else if (item?.dbProperty == "specialtesting.phenotypeResult") {
                    tempFilterLabels['phenoType-name'] = {
                        keyName: 'name',
                        label: 'PhenoType'
                    };
                }
    ...
    filtersNewDataKeys =
        filtersNewDataKeys &&
        filtersNewDataKeys.map((item) => {
            if (requestBatchfiltersData?.length > 0 && requestBatchfiltersData?.data[0][item][0]?.type === 'range') {
                console.log('range--', requestBatchfiltersData?.data[0][item][0])
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [requestBatchfiltersData.data[0][item][0].min, requestBatchfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...requestBatchfiltersData.data[0][item][0]
            ...
            if (requestBatchfiltersData.length > 0 && requestBatchfiltersData?.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filtersNewDataValues =
        requestBatchfiltersData && requestBatchfiltersData.data
            ? {
                ...requestBatchfiltersData.data[0]
            }
            : {};

    let filtersNewDataFiveValues = {};
    ...
    requestBatchfiltersData &&
        requestBatchfiltersData.data &&
        filtersNewDataKeys &&
        filtersNewDataKeys.forEach((i) => {
            if (typeof i === 'string') {
                if (i === 'bloodgroupId-name') {
                    allFilterValues[i] = [];
                    filtersNewDataValues[i].forEach((item) => {
                    ...
                    allFilterValues[i] = [...new Set(filtersNewDataValues[i])]
                        .map((item) => {
    ...
    if (
        ...
        filtersNewDataKeys &&
        !filtersNewDataKeys?.includes('Expiry')
    ) {
        filtersNewDataKeys.push({
            type: 'Expiry',
            name: 'Expiry',
            filters: [
    ...
    useEffect(() => {
        if (chipData.length > 0) {
            if (dateItem && dateItem.length > 1) {
                ...
                let switchLable = filtersNewDataKeys.filter((item) => item?.name === switchItem)[0];
    ...
    function filterSubmit() {
        ...
        let filterKeysObjects = filtersNewDataKeys.filter((item) => typeof item === 'object');
    ...
    const handleChipChanges = (event, filterName, nodeNumber) => {
        setExpandedNodes([nodeNumber]);
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem) {
                    let dateName = filtersNewDataKeys.filter((item) => item?.label === dateItem)[0];
                ...
                if (currentItem) {
                    ...
                    let rangeLable = filtersNewDataKeys.filter((item) => item?.label === currentItem)[0];
                    let tempFiltersDate = [
                ...
                if (isChecked) {
                    ...
                    let tempChipIdAndName = {};
                    console.log(props?.chipIdAndName, 'outSi');
                    if (props?.chipIdAndName) {
                        tempChipIdAndName = { ...props?.chipIdAndName, ...chipNameAndId };
                    } else {
                        tempChipIdAndName = { ...chipNameAndId };
                    }
                    tempChipIdAndName[chipName] = value.id;
                    setChipNameAndId(tempChipIdAndName);
                    console.log("filters")
                    if (firtHalfKey === "phenoType") {
                        if (value.name != null && value.name.length > 0) {
                            let tempFiltersDate = [...filtersData, { key: "specialtesting.phenotype", value: [value.name] }];
                            setFiltersData([...tempFiltersDate]);
                        }
                    }
                    else if (filtersData.findIndex((item) => item.key === `${firtHalfKey}._id`) === -1) {
                        console.log('item--', filtersData)
    ...
    const handleSwitchChange = (e, feild) => {
        ...
        let switchLable = filtersNewDataKeys.filter((item) => item?.name === feild)[0];
        let tempChipData = chipData.map((item) => {
    ...
    console.log(filtersNewDataKeys, allFilterValues, 'requestBatchfiltersData');

    return (
        <RequestBatchFilterComponent
            ...
            showFilters={filtersNewDataKeys}
            filtersObject={filtersNewDataFiveValues}
            ...
            filtersLoading={requestBatchfiltersLoading}
            enableFilter={enableFilter}
            staticGroups={isRequestBatch ? staticGroups : ''}
            isRequestBatch={isRequestBatch}
            noData={requestBatchfiltersData?.data?.length === 0}
        />
    );
};

export default RequestBatchFilter;
