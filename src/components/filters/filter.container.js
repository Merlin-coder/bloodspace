...
const Filter = (props) => {
    ...
    console.log('datefilter', datesObject)
    ...
    let filterCriteriaKeys = filterCriteria && filterCriteria.data?.length > 0 && Object.keys(filterCriteria?.data[0]);
    ...
    useEffect(() => {
        if (
            filterCriteria &&
            filterCriteria.data &&
            filterCriteria.data?.length > 0 &&
            responseData &&
            responseData.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(filterCriteria?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', filterCriteria)
            tempFilterKeys.forEach((item) => {
                console.log('tempkeysitems', item, )
                if (filterCriteria?.data?.length > 0 && filterCriteria.data[0][item][0]?.type === 'date' && item === 'createdAt' || item === 'dob' ) {
                    ...
                    console.log('dateitemllabel', dateItemLabel)
                        ...
                        let dateItem = {
                            label: dateLabel,
                            name: item,
                            startDate: moment(new Date(filterCriteria.data[0][item][0].min)).format('YYYY-MM-DD'),
                            endDate: moment(new Date(filterCriteria.data[0][item][0].max)).format('YYYY-MM-DD'),
                            minDate: moment(new Date(filterCriteria.data[0][item][0].min)).format('YYYY-MM-DD'),
                            maxDate: moment(new Date(filterCriteria.data[0][item][0].max)).format('YYYY-MM-DD')
                        };
                        console.log('dateitem---', dateItem)
                        tempDatesObject[item] = dateItem;
                   
                }
                if (filterCriteria.length > 0 && filterCriteria.data[0][item][0]?.type === 'switch') {
    ...
    filterCriteriaKeys.map((item) => {
        if (filterCriteria.length > 0 && filterCriteria?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
            range[item] = [filterCriteria.data[0][item][0].min, filterCriteria.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...filterCriteria.data[0][item][0]
                };
                return rangeItem;
            }
        if (filterCriteria.length > 0 && filterCriteria.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        filterCriteria && filterCriteria.data
            ? {
                ...filterCriteria.data[0]
              }
            : {};
    ...
    const handleChipChanges = (event, filterName) => {
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem) {
                    let dateName = filterCriteriaKeys.filter((item) => item?.name === dateItem);
