...
const UserFilter = (props) => {
    ...
    let { userfiltersLoading, userfiltersData } = useSelector((state) => state.getUserFilter);
    ...
    let filterCriteriaKeys = userfiltersData && userfiltersData?.data?.length > 0 && Object.keys(userfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            userfiltersData &&
            userfiltersData?.data &&
            userfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(userfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', userfiltersData)
            tempFilterKeys.forEach((item) => {
                console.log('itemmmm', item)
                //console.log('tempkeysitems', item, userfiltersData && userfiltersData?.data[0][item][0]?.type)
                if (userfiltersData &&
                    userfiltersData.data &&
                    userfiltersData.data[0] &&
                    userfiltersData.data[0][item] &&
                    userfiltersData.data[0][item][0] &&
                    userfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt' || item === 'accountValidity') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(userfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(userfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(userfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(userfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (userfiltersData?.length > 0 && userfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (userfiltersData.length > 0 && userfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [userfiltersData.data[0][item][0].min, userfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...userfiltersData.data[0][item][0]
            ...
            if (userfiltersData.length > 0 && userfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        userfiltersData && userfiltersData.data
            ? {
                ...userfiltersData.data[0]
            }
            : {};
    ...
    const handleChipChanges = (event, filterName) => {
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem === 'Activity At') {
                ...
                else if (dateItem === 'Account Validity') {
                    let dateName = filterCriteriaKeys.filter((item) => item?.label === dateItem)[0];
                    let findDate = chipData
                        .filter((item) => item.includes(dateItem))[0]
                        .split(' ')
                    ...
                    let tempFiltersDate = [
                        ...filtersData.filter((item) => item.key !== dateName?.name),
                        { key: 'accountValidity', value: [findDate[0], findDate[1]] }
                    ];
                    setFiltersData([...tempFiltersDate]);
                }
    ...
    return (
        <UserFilterComponent
            ...
            filtersLoading={userfiltersLoading}
