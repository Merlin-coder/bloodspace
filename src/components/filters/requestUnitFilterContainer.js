import RequestUnitFilterComponent from './requestUnitFilterComponent';
...
const RequestUnitFilter = (props) => {
    ...
    let filtersNewDataKeys = filtersNewData && filtersNewData.data?.length > 0 && Object.keys(filtersNewData?.data[0]);
    ...
    useEffect(() => {
        if (
            filtersNewData &&
            filtersNewData.data &&
            filtersNewData.data?.length > 0 &&
            responseData &&
            responseData.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(filtersNewData?.data[0]);
            ...
            tempFilterKeys.forEach((item) => {
                if (filtersNewData &&
                    filtersNewData.data &&
                    filtersNewData.data[0] &&
                    filtersNewData.data[0][item] &&
                    filtersNewData.data[0][item][0] &&
                    filtersNewData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    console.log('iffffff', item)
                    let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                    let dateLabel = dateItemLabel?.label;
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(filtersNewData.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(filtersNewData.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(filtersNewData.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(filtersNewData.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    tempDatesObject[item] = dateItem;
                }
                if (filtersNewData.length > 0 && filtersNewData?.data[0][item][0]?.type === 'switch') {
            ...
            if (isRequestUnit) {
                ...
                let expiryDateTimeObject = filtersNewDataKeys?.filter((i) => i.name === 'expiryDateAndTime')[0];
                let dereservationDateObject = filtersNewDataKeys?.filter((i) => i.name === 'dereservationDate')[0];
                let tempStaticGroups = [
                    {
                        name: 'Blood Unit',
                        data: [
                            'productcodeId-isbtcode',
                            'productcodeId-isbtdescription',
                            'bloodgroupId-name',
                            'productgroupId-name',
                            'phenoType-name',
    ...
    filtersNewDataKeys =
       ...
       filtersNewDataKeys.map((item) => {
           if (filtersNewData?.length > 0 && filtersNewData?.data[0][item][0]?.type === 'range') {
               console.log('range--', filtersNewData?.data[0][item][0])
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
               range[item] = [filtersNewData.data[0][item][0].min, filtersNewData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...filtersNewData.data[0][item][0]
           ...
           if (filtersNewData.length > 0 && filtersNewData?.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filtersNewDataValues =
        filtersNewData && filtersNewData.data
            ? {
                ...filtersNewData.data[0]
              }
            : {};
    ...
    console.log(filtersNewDataKeys, allFilterValues, 'filtersNewData');

    return (
        <RequestUnitFilterComponent
            ...
            filtersLoading={newfiltersLoading}
            enableFilter={enableFilter}
            staticGroups={isRequestUnit ? staticGroups : ''}
            isRequestUnit={isRequestUnit}
            noData={filtersNewData?.data?.length === 0}
        />
    );
};

export default RequestUnitFilter;
