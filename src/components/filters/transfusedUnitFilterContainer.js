import TransfusedUnitFilterComponent from './transfusedUnitFilterComponent';
...
const TransfusedUnitFilter = (props) => {
    ...
    let filterCriteriaKeys = transfusedUnitfiltersData && transfusedUnitfiltersData?.data?.length > 0 && Object.keys(transfusedUnitfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            transfusedUnitfiltersData &&
            transfusedUnitfiltersData?.data &&
            transfusedUnitfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(transfusedUnitfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', transfusedUnitfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, transfusedUnitfiltersData && transfusedUnitfiltersData?.data[0][item][0]?.type)
                if (transfusedUnitfiltersData &&
                    transfusedUnitfiltersData.data &&
                    transfusedUnitfiltersData.data[0] &&
                    transfusedUnitfiltersData.data[0][item] &&
                    transfusedUnitfiltersData.data[0][item][0] &&
                    transfusedUnitfiltersData.data[0][item][0].type === 'date' &&
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(transfusedUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(transfusedUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(transfusedUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(transfusedUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (transfusedUnitfiltersData?.length > 0 && transfusedUnitfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [transfusedUnitfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (transfusedUnitfiltersData.length > 0 && transfusedUnitfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [transfusedUnitfiltersData.data[0][item][0].min, transfusedUnitfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...transfusedUnitfiltersData.data[0][item][0]
            ...
            if (transfusedUnitfiltersData.length > 0 && transfusedUnitfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        transfusedUnitfiltersData && transfusedUnitfiltersData.data
            ? {
                ...transfusedUnitfiltersData.data[0]
            }
            : {};
    ...
    return (
        <TransfusedUnitFilterComponent
            ...
            filtersLoading={transfusedUnitfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default TransfusedUnitFilter;
