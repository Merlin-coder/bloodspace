import WastedBatchFilterComponent from './wastedBatchFilterComponent';
...
const WastedBatchFilter = (props) => {
    ...
    let { wastedBatchfiltersLoading, wastedBatchfiltersData } = useSelector((state) => state.getWastedBatchFilter);
    console.log('wastedBatchfiltersData', wastedBatchfiltersData);
    ...
    let filterCriteriaKeys = wastedBatchfiltersData && wastedBatchfiltersData?.data?.length > 0 && Object.keys(wastedBatchfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            wastedBatchfiltersData &&
            wastedBatchfiltersData?.data &&
            wastedBatchfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(wastedBatchfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', wastedBatchfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, wastedBatchfiltersData && wastedBatchfiltersData?.data[0][item][0]?.type)
                if (wastedBatchfiltersData &&
                    wastedBatchfiltersData.data &&
                    wastedBatchfiltersData.data[0] &&
                    wastedBatchfiltersData.data[0][item] &&
                    wastedBatchfiltersData.data[0][item][0] &&
                    wastedBatchfiltersData.data[0][item][0].type === 'date' &&
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(wastedBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(wastedBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(wastedBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(wastedBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (wastedBatchfiltersData?.length > 0 && wastedBatchfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [wastedBatchfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (wastedBatchfiltersData.length > 0 && wastedBatchfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [wastedBatchfiltersData.data[0][item][0].min, wastedBatchfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...wastedBatchfiltersData.data[0][item][0]
            ...
            if (wastedBatchfiltersData.length > 0 && wastedBatchfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        wastedBatchfiltersData && wastedBatchfiltersData.data
            ? {
                ...wastedBatchfiltersData.data[0]
            }
            : {};
    ...
    return (
        <WastedBatchFilterComponent
            ...
            filtersLoading={wastedBatchfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default WastedBatchFilter;
