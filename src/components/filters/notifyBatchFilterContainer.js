import NotifyBatchFilterComponent from './notifyBatchFilterComponent';
...
const NotifyBatchFilter = (props) => {
    ...
    console.log('notifybatchfiltersdata', notifyBatchfiltersData);
    ...
    let filterCriteriaKeys = notifyBatchfiltersData && notifyBatchfiltersData?.data?.length > 0 && Object.keys(notifyBatchfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            notifyBatchfiltersData &&
            notifyBatchfiltersData?.data &&
            notifyBatchfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(notifyBatchfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', notifyBatchfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, notifyBatchfiltersData && notifyBatchfiltersData?.data[0][item][0]?.type)
                if (notifyBatchfiltersData &&
                    notifyBatchfiltersData.data &&
                    notifyBatchfiltersData.data[0] &&
                    notifyBatchfiltersData.data[0][item] &&
                    notifyBatchfiltersData.data[0][item][0] &&
                    notifyBatchfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(notifyBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(notifyBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(notifyBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(notifyBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (notifyBatchfiltersData?.length > 0 && notifyBatchfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [notifyBatchfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (notifyBatchfiltersData.length > 0 && notifyBatchfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [notifyBatchfiltersData.data[0][item][0].min, notifyBatchfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...notifyBatchfiltersData.data[0][item][0]
            ...
            if (notifyBatchfiltersData.length > 0 && notifyBatchfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        notifyBatchfiltersData && notifyBatchfiltersData.data
            ? {
                ...notifyBatchfiltersData.data[0]
            }
            : {};
    ...
    return (
        <NotifyBatchFilterComponent
            ...
            filtersLoading={notifyBatchfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default NotifyBatchFilter;
