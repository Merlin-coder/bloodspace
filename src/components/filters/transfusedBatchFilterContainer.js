import TransfusedBatchFilterComponent from './transfusedBatchFilterComponent';
...
const TransfusedBatchFilter = (props) => {
    ...
    let { transfusedUnitfiltersLoading, transfusedUnitfiltersData } = useSelector((state) => state.getTransfusedUnitFilter);
    console.log('transfusedUnitfiltersData', transfusedUnitfiltersData);

    let { transfusedBatchfiltersLoading, transfusedBatchfiltersData } = useSelector((state) => state.getTransfusedBatchFilter);
    console.log('transfusedBatchfiltersData', transfusedBatchfiltersData);
    ...
    let filterCriteriaKeys = transfusedBatchfiltersData && transfusedBatchfiltersData?.data?.length > 0 && Object.keys(transfusedBatchfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            transfusedBatchfiltersData &&
            transfusedBatchfiltersData?.data &&
            transfusedBatchfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(transfusedBatchfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', transfusedBatchfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, transfusedBatchfiltersData && transfusedBatchfiltersData?.data[0][item][0]?.type)
                if (transfusedBatchfiltersData && transfusedBatchfiltersData?.data && transfusedBatchfiltersData?.data[0]?.length > 0
                    && transfusedBatchfiltersData && transfusedBatchfiltersData?.data && transfusedBatchfiltersData?.data[0][item][0]?.type === 'date' && item === 'createdAt' || item === 'expiryDate') {
                    console.log('iffffff', displayConfData)
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(transfusedBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(transfusedBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(transfusedBatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(transfusedBatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (transfusedBatchfiltersData?.length > 0 && transfusedBatchfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [transfusedBatchfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (transfusedBatchfiltersData.length > 0 && transfusedBatchfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [transfusedBatchfiltersData.data[0][item][0].min, transfusedBatchfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...transfusedBatchfiltersData.data[0][item][0]
                };
                return rangeItem;
            }
            if (transfusedBatchfiltersData.length > 0 && transfusedBatchfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        transfusedBatchfiltersData && transfusedBatchfiltersData.data
            ? {
                ...transfusedBatchfiltersData.data[0]
            }
            : {};
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem === 'Activity At') {
                    ...
                    let tempFiltersDate = [
                        ...filtersData.filter((item) => item.key !== dateName?.name),
                        { key: 'expiryDate', value: [findDate[0], findDate[1]] }
                    ];
                    setFiltersData([...tempFiltersDate]);
                }
    ...
    return (
        <TransfusedBatchFilterComponent
            ...
            filtersLoading={transfusedBatchfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default TransfusedBatchFilter;
