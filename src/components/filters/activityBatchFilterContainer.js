import ActivityBatchFilterComponent from './activityBatchFilterComponent';
...
const ActivityBatchFilter = (props) => {
    ...
    let { activityBatchfiltersLoading, activitybatchfiltersData } = useSelector((state) => state.getActivityBatchFilter);
    console.log('activitybatchfiltersData', activitybatchfiltersData);
    ...
    let filterCriteriaKeys = activitybatchfiltersData && activitybatchfiltersData?.data?.length > 0 && Object.keys(activitybatchfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            activitybatchfiltersData &&
            activitybatchfiltersData?.data &&
            activitybatchfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(activitybatchfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', activitybatchfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, activitybatchfiltersData && activitybatchfiltersData?.data[0][item][0]?.type)
                if (activitybatchfiltersData &&
                    activitybatchfiltersData.data &&
                    activitybatchfiltersData.data[0] &&
                    activitybatchfiltersData.data[0][item] &&
                    activitybatchfiltersData.data[0][item][0] &&
                    activitybatchfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(activitybatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(activitybatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(activitybatchfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(activitybatchfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (activitybatchfiltersData?.length > 0 && activitybatchfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [activitybatchfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (activitybatchfiltersData.length > 0 && activitybatchfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [activitybatchfiltersData.data[0][item][0].min, activitybatchfiltersData?.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...activitybatchfiltersData?.data[0][item][0]
            ...
            if (activitybatchfiltersData?.length > 0 && activitybatchfiltersData?.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        activitybatchfiltersData && activitybatchfiltersData?.data
            ? {
                ...activitybatchfiltersData?.data[0]
            }
            : {};
    ...
    return (
        <ActivityBatchFilterComponent
            ...
            filtersLoading={activityBatchfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default ActivityBatchFilter;
