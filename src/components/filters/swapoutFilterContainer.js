import SwapoutFilterComponent from './swapoutFilterComponent';
...
const SwapoutFilter = (props) => {
    ...
    console.log('STATIC',staticGroups)
    ...
    let { swapoutfiltersLoading, swapoutfiltersData } = useSelector((state) => state.getSwapoutFilter);
    ...
    let filterCriteriaKeys = swapoutfiltersData && swapoutfiltersData?.data?.length > 0 && Object.keys(swapoutfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            swapoutfiltersData &&
            swapoutfiltersData?.data &&
            swapoutfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(swapoutfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', swapoutfiltersData)
            tempFilterKeys.forEach((item) => {
                console.log('itemmmm', item)
                //console.log('tempkeysitems', item, swapoutfiltersData && swapoutfiltersData?.data[0][item][0]?.type)
                if (swapoutfiltersData &&
                    swapoutfiltersData.data &&
                    swapoutfiltersData.data[0] &&
                    swapoutfiltersData.data[0][item] &&
                    swapoutfiltersData.data[0][item][0] &&
                    swapoutfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(swapoutfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(swapoutfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(swapoutfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(swapoutfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (swapoutfiltersData?.length > 0 && swapoutfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [swapoutfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (swapoutfiltersData.length > 0 && swapoutfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [swapoutfiltersData.data[0][item][0].min, swapoutfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...swapoutfiltersData.data[0][item][0]
            ...
            if (swapoutfiltersData.length > 0 && swapoutfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        swapoutfiltersData && swapoutfiltersData.data
            ? {
                ...swapoutfiltersData.data[0]
            }
            : {};
    ...
    return (
        <SwapoutFilterComponent
            ...
            filtersLoading={swapoutfiltersLoading}
