import NotifyUnitFilterComponent from './notifyUnitFilterComponent';
...
const NotifyUnitFilter = (props) => {
    ...
    let filterCriteriaKeys = notifyUnitfiltersData && notifyUnitfiltersData?.data?.length > 0 && Object.keys(notifyUnitfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            notifyUnitfiltersData &&
            notifyUnitfiltersData?.data &&
            notifyUnitfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(notifyUnitfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', notifyUnitfiltersData)
            tempFilterKeys.forEach((item) => {
                //console.log('tempkeysitems', item, notifyUnitfiltersData && notifyUnitfiltersData?.data[0][item][0]?.type)
                if (notifyUnitfiltersData &&
                    notifyUnitfiltersData.data &&
                    notifyUnitfiltersData.data[0] &&
                    notifyUnitfiltersData.data[0][item] &&
                    notifyUnitfiltersData.data[0][item][0] &&
                    notifyUnitfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt' || item === 'ackDateAndTime') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(notifyUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(notifyUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(notifyUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(notifyUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (notifyUnitfiltersData?.length > 0 && notifyUnitfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [notifyUnitfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (notifyUnitfiltersData.length > 0 && notifyUnitfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [notifyUnitfiltersData.data[0][item][0].min, notifyUnitfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...notifyUnitfiltersData.data[0][item][0]
            ...
            if (notifyUnitfiltersData.length > 0 && notifyUnitfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        notifyUnitfiltersData && notifyUnitfiltersData.data
            ? {
                ...notifyUnitfiltersData.data[0]
            }
            : {};
    ...
    const handleChipChanges = (event, filterName) => {
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem === 'Activity At') {
                ...
                else if (dateItem === 'Ack Date & Time') {
                    let dateName = filterCriteriaKeys.filter((item) => item?.label === dateItem)[0];
                    let findDate = chipData
                        .filter((item) => item.includes(dateItem))[0]
                        .split(' ')
                    ...
                    let tempFiltersDate = [
                        ...filtersData.filter((item) => item.key !== dateName?.name),
                        { key: 'ackDateAndTime', value: [findDate[0], findDate[1]] }
                    ];
                    setFiltersData([...tempFiltersDate]);
                }
    ...
    return (
        <NotifyUnitFilterComponent
            ...
            filtersLoading={notifyUnitfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default NotifyUnitFilter;
