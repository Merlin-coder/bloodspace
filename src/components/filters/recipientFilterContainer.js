import RecipientFilterComponent from './recipientFilterComponent';
...
const RecipientFilter = (props) => {
    ...
    let { recipientfiltersLoading, recipientfiltersData } = useSelector((state) => state.getRecipientFilter);
    console.log('recipientfiltersData', recipientfiltersData);
    ...
    console.log('connnnn', displayConfData)
    //filterLabels is an object to display labels in filters and values
    const [filterLabels, setFilterLabels] = useState({});
    // filterCriteria = filterResponseData;

    let filterCriteriaKeys = recipientfiltersData && recipientfiltersData?.data?.length > 0 && Object.keys(recipientfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            recipientfiltersData &&
            recipientfiltersData?.data &&
            recipientfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(recipientfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', recipientfiltersData)
            tempFilterKeys.forEach((item) => {
                console.log('itemmmm', item)
                //console.log('tempkeysitems', item, recipientfiltersData && recipientfiltersData?.data[0][item][0]?.type)
                if (recipientfiltersData &&
                    recipientfiltersData.data &&
                    recipientfiltersData.data[0] &&
                    recipientfiltersData.data[0][item] &&
                    recipientfiltersData.data[0][item][0] &&
                    recipientfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt' || item === 'dob') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(recipientfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(recipientfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(recipientfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(recipientfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (recipientfiltersData?.length > 0 && recipientfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [recipientfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (recipientfiltersData.length > 0 && recipientfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [recipientfiltersData.data[0][item][0].min, recipientfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...recipientfiltersData.data[0][item][0]
            ...
            if (recipientfiltersData.length > 0 && recipientfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        recipientfiltersData && recipientfiltersData.data
            ? {
                ...recipientfiltersData.data[0]
            }
            : {};
    ...
    const handleChipChanges = (event, filterName) => {
        ...
        if (filterName === 'Expiry') {
            ...
            if (event === 'switchType') {
                ...
                if (dateItem === 'Activity At') {
                ...
                else if (dateItem === 'Date of Birth') {
                    let dateName = filterCriteriaKeys.filter((item) => item?.label === dateItem)[0];
                    let findDate = chipData
                        .filter((item) => item.includes(dateItem))[0]
                        .split(' ')
                    ...
                    let tempFiltersDate = [
                        ...filtersData.filter((item) => item.key !== dateName?.name),
                        { key: 'dob', value: [findDate[0], findDate[1]] }
                    ];
                    setFiltersData([...tempFiltersDate]);
                }
    ...
    return (
        <RecipientFilterComponent
            ...
            filtersLoading={recipientfiltersLoading}
