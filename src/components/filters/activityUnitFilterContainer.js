import ActivityUnitFilterComponent from './activityUnitFilterComponent';
...
const ActivityUnitFilter = (props) => {
    ...
    let filterCriteriaKeys = activityUnitfiltersData && activityUnitfiltersData?.data?.length > 0 && Object.keys(activityUnitfiltersData?.data[0]);
    ...
    useEffect(() => {
        if (
            activityUnitfiltersData &&
            activityUnitfiltersData?.data &&
            activityUnitfiltersData?.data?.length > 0 &&
            responseData &&
            responseData?.displayConfigData
        ) {
            let tempFilterKeys = Object.keys(activityUnitfiltersData?.data[0]);

            let tempDatesObject = {};
            let tempSwitchObject = {};
            let tempRowData = {};
            console.log('filtercriterrrrrr', activityUnitfiltersData)
            tempFilterKeys.forEach((item) => {
                console.log('itemmmm',item)
                //console.log('tempkeysitems', item, activityUnitfiltersData && activityUnitfiltersData?.data[0][item][0]?.type)
                if (activityUnitfiltersData &&
                    activityUnitfiltersData.data &&
                    activityUnitfiltersData.data[0] &&
                    activityUnitfiltersData.data[0][item] &&
                    activityUnitfiltersData.data[0][item][0] &&
                    activityUnitfiltersData.data[0][item][0].type === 'date' &&
                    item === 'createdAt') {
                    ...
                    let dateItem = {
                        label: dateLabel,
                        name: item,
                        startDate: moment(new Date(activityUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        endDate: moment(new Date(activityUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD'),
                        minDate: moment(new Date(activityUnitfiltersData?.data[0][item][0].min)).format('YYYY-MM-DD'),
                        maxDate: moment(new Date(activityUnitfiltersData?.data[0][item][0].max)).format('YYYY-MM-DD')
                    };
                    console.log('dateitem---', dateItem)
                    tempDatesObject[item] = dateItem;

                }
                if (activityUnitfiltersData?.length > 0 && activityUnitfiltersData?.data[0][item][0]?.type === 'switch') {
    ...
    }, [activityUnitfiltersData, responseData]);
    useEffect(() => {
    ...
    filterCriteriaKeys =
        ...
        filterCriteriaKeys.map((item) => {
            if (activityUnitfiltersData.length > 0 && activityUnitfiltersData?.data[0][item][0]?.type === 'range') {
                let rangeItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let rangeLabel = rangeItemLabel?.label;
                range[item] = [activityUnitfiltersData.data[0][item][0].min, activityUnitfiltersData.data[0][item][0].max];
                let rangeItem = {
                    name: item,
                    label: rangeLabel,
                    ...activityUnitfiltersData.data[0][item][0]
            ...
            if (activityUnitfiltersData.length > 0 && activityUnitfiltersData.data[0][item][0]?.type === 'date') {
                let dateItemLabel = displayConfData && displayConfData.find((a) => a.dbProperty === item);
                let dateLabel = dateItemLabel?.label;
                let rangeItem = {
                    label: dateLabel,
    ...
    let filterCriteriaValues =
        activityUnitfiltersData && activityUnitfiltersData.data
            ? {
                ...activityUnitfiltersData.data[0]
            }
            : {};
    ...
    return (
        <ActivityUnitFilterComponent
            ...
            filtersLoading={activityUnitfiltersLoading}
            enableFilter={enableFilter}
        />
    );
};

export default ActivityUnitFilter;
